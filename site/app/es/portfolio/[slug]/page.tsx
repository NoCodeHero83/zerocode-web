...
export async function generateStaticParams() {
  ...
  return portfolioItemsData.es.map((item) => ({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const item = getPortfolioItemBySlug(slug, 'es')
  if (!item) return {}
  return {
    title: `${item.title} — Portafolio | Zerocode`,
    description: item.description,
  }
}

export default async function PortfolioItemPageEs({ params }: Props) {
  const { slug } = await params
  const item = getPortfolioItemBySlug(slug, 'es')
  if (!item) notFound()

  const shared = homeContent.es
  const relatedItems = getRelatedItems(item, 'es')
    ...
    <>
      ...
      <main className="relative overflow-hidden">
        ...
        <section className="pb-16 sm:pb-24 lg:pb-28 relative -mt-12 sm:-mt-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
            <div className="flex flex-col lg:flex-row gap-8 p-5 lg:p-7 border border-white/10 rounded-[25px] bg-[#FFFFFF1C] shadow-[0_0_0_0_rgba(0,0,0,0)]">
          ...
          <section className="py-16 sm:py-24 lg:py-28 relative">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
              <div className="mx-auto mb-12 max-w-3xl text-center sm:mb-16">
                <h2 className="text-3xl font-bold text-balance text-foreground sm:text-4xl">Proyectos Relacionados</h2>
              </div>
