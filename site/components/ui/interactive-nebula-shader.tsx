...
/**
 * Full-bleed nebula shader background.
 * Props drive three GLSL uniforms — no demo markup here.
 * By default it is absolutely positioned and sits behind its parent
 * content (use inside a `relative` hero section, not as a global fixed layer).
 */
export function InteractiveNebulaShader({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void mainImage(out vec4 O, in vec2 fragCoord) {
        ...
        // Center dimming (keeps overlaid text readable)
        float dist   = distance(fragCoord, iResolution*0.5);
        float radius = min(iResolution.x, iResolution.y) * 0.5;
        float dim    = disableCenterDimming
                     ? 1.0
                     : smoothstep(radius*0.3, radius*0.6, dist);

        O = vec4(col, 1.0);
        if (!disableCenterDimming) {
          O.rgb = mix(O.rgb * 0.25, O.rgb, dim);
        }
      }
  ...
  return (
    <div
      ref={containerRef}
      className={`pointer-events-none absolute inset-0 -z-10 bg-background ${className}`}
      aria-hidden="true"
    />
  )
}
